      ...
      eventInvites.forEach((invite) => {
        invite.guests.forEach((guest, index) => {
          participants.push({
            ...
            mainGuest:
              invite.mainGuest.firstName + ' ' + invite.mainGuest.lastName,
          ...
          return (
            ...
            >
              <div className='flex flex-row justify-start flex-wrap'>
                ...
                {participant.name != participant.mainGuest && (
                  <div className='mx-2'>{participant.mainGuest}'s Guest</div>
                )}
              </div>
              <div className='flex flex-row items-center'>
                ...
                <div
                  className={
                    (participant.rsvp ? 'bg-green-600' : 'bg-red-600') +
                    ' italic border border-black mr-2  w-4 h-4 rounded-full'
                  }
                ></div>
                <div className=' text-blue-800'>
                  <Link href='./app'>Invite</Link>
                </div>
              </div>
            </div>
          );
